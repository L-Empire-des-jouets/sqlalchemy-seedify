...
class FakeCustomerSeeder(BaseSeeder):
    ...
    def rollback(self):
        ...
        deleted = (
            ...
            .filter(Customer.registration_date >= two_years_ago)
    ...
    def run(self):
        ...
        for i in range(num_products):
            product = Product(
                ...
                sale_price=round(random.uniform(8.99, 899.99), 2)
    ...
    def run(self):
        ...
        for i in range(num_orders):
            ...
            order = Order(
                ...
                delivered_date=order_date + timedelta(days=random.randint(4, 7))
    ...
    def rollback(self):
        ...
        self.session.query(OrderItem).filter(
            OrderItem.order_id.in_(
                self.session.query(Order.id).filter(Order.order_number.like("ORD-%"))
            )
        ).delete(synchronize_session=False)
        ...
        deleted = (
            ...
            .filter(Order.order_number.like("ORD-%"))
            .delete(synchronize_session=False)
        )
