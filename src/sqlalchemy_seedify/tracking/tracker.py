...
class SeederTracker:
    ...
    def mark_failed(
        ...
        logger.error(
            f"Marked seeder {seeder_name} as failed in environment {environment}: {error_message}"
        )
    ...
    def mark_rolled_back(self, seeder_name: str) -> None:
        ...
        Args:
            seeder_name: Name of the seeder
        """
        records = self.session.query(SeederRecord).filter_by(seeder_name=seeder_name).all()
    ...
    def get_record(self, seeder_name: str, environment: str) -> Optional[SeederRecord]:
        """
    ...
    def get_executed_seeders(self, environment: Optional[str] = None) -> List[SeederRecord]:
        """
    ...
    def get_pending_seeders(self, all_seeders: List[str], environment: str) -> List[str]:
        """
    ...
    def get_next_batch(self) -> int:
        ...
        """
        max_batch = self.session.query(func.max(SeederRecord.batch)).scalar() or 0
        return max_batch + 1
    ...
    def clear_history(self, environment: Optional[str] = None, force: bool = False) -> int:
        """
        ...
        """
        if not force:
            logger.warning("Clearing history requires force=True to prevent accidental deletion")
            return 0
